<!DOCTYPE html>
<html>
    <head>
        <title>Three Divs HTML</title>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0">
 
	<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.4/css/bootstrap.min.css">       
        <link rel="stylesheet" href="/02bprove/stylesheet.css" media="screen">       
 
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js"></script>
	<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.4/js/bootstrap.min.js"></script>  
    </head>

<!-- COMMENT:   begin webpage  -->
<body>
<div class="container">
    
<!-- COMMENT:  banner     -->
<?php include 'modules/responsive-header.php'; ?>

<!-- COMMENT:  navigation     -->
<?php include 'modules/responsive-nav.php'; ?>

<div class="row"> 
<main><div>   

<!-- Shopping Cart HTML   --> 
    <div class="info"  >       
        <div class="innermargins">
            <h2 style="text-align:left">Assignment:  Shopping Cart</h2>
            <a href="https://peaceful-oasis-06205.herokuapp.com/03prove/homepage.php">Visit my site.</a>
        </div>
    </div>
    
</div></main>
</div>

<!-- COMMENT:  footer     -->
<footer>
 <?php include 'modules/responsive-footer-nav.php'; ?>
</footer>

</div>
</body>
</html>
